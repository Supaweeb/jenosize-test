    ...
    for (let operator1 of operator) {
      for (let operator2 of operator) {
        for (let operator3 of operator) {
          const exprs = [
            `(${p[0]}${operator1}${p[1]})${operator2}${p[2]}${operator3}${p[3]}`,
            `${p[0]}${operator1}(${p[1]}${operator2}${p[2]})${operator3}${p[3]}`,
            `${p[0]}${operator1}${p[1]}${operator2}(${p[2]}${operator3}${p[3]})`,
            `${p[0]}${operator1}${p[1]}${operator2}${p[2]}${operator3}${p[3]}`
          ]
...
exports.checkGame = () => (req, res, next) => {
  if (!req.query.number || req.query.number.length !== 4) {
    res.status(400).json({ message: "Missing params number or length of number is not 4 digit" });
  }
  const numbers = req.query.number.split("")
  res.json(make24(...numbers) ? "YES" : "NO")
}
